...
//     if (!size || size <= 0) {
//       document.getElementById('result').innerHTML = 'Введите корректный размер';
//       document.getElementById('showTableBtn').style.display = 'none';
//       return;
//     }
  
//     // Загружаем данные из кассеты
//     const fits = await loadKnowledgeData('fits.json');
    
//     const table = fits[fit];
//     if (!table) {
//       document.getElementById('result').innerHTML = 'Квалитет не найден';
//       document.getElementById('showTableBtn').style.display = 'none';
//       return;
//     }
  
//     const row = table.find(item => size > item.min && size <= item.max);
//     if (!row) {
//       document.getElementById('result').innerHTML = 'Размер вне диапазона';
//       document.getElementById('showTableBtn').style.display = 'none';
//       return;
//     }
...
//     const diff = ((row.upper - row.lower) / 1000).toFixed(3);
  
//     document.getElementById('result').innerHTML = `
//       <strong>Ø${sizeInput}${fit}:</strong><br>
//       Верхнее отклонение: ${upper} мм<br>
//       Нижнее отклонение: ${lower} мм<br>
//       Поле допуска: ${diff} мм
//     `;
//     document.getElementById('showTableBtn').style.display = 'block';
//   }

// Обновление информации о посадке
...
// speak(speechText);
//     // Добавь озвучку:
// const speechText = `Размер ${size} миллиметров, посадка ${fit}. 
// Верхнее отклонение: ${upper >= 0 ? 'плюс' : 'минус'} ${Math.abs(upperStr)} миллиметров. 
// Нижнее отклонение: ${lower >= 0 ? 'плюс' : 'минус'} ${Math.abs(lowerStr)} миллиметров. 
// Поле допуска: ${diffStr} миллиметров.`;
...
//   const utter = new SpeechSynthesisUtterance(text);
//   utter.lang = 'ru-RU';
//   utter.rate = 1.3;   // Быстрее!
//   utter.pitch = 1.1;  // Чуть выше — звучит как "профессионал"
//   const voices = window.speechSynthesis.getVoices();
//   const voice = voices.find(v => v.lang === 'ru-RU') || voices[0];
...
//   // Создаём новый голосовой юнит
//   const utter = new SpeechSynthesisUtterance(text);
  
//   // Настройки
//   utter.lang = 'ru-RU';           // язык
//   utter.rate = 0.9;               // скорость (1 — норма)
//   utter.pitch = 1;                // тон

//   // Можно выбрать голос (по желанию)
//   const voices = window.speechSynthesis.getVoices();
//   const voice = voices.find(v => v.lang === 'ru-RU');
//   if (voice) {
//     utter.voice = voice;
//   }
...
//   if (!tolerances[fit]) {
//     document.getElementById('result').innerHTML = `
//       <div class="error">❌ Посадка <strong>${fit.toUpperCase()}</strong> не найдена в базе данных.</div>
//     `;
//     return;
//   }
...
//   // 4. Поиск интервала
//   const intervalRow = rows.find(r => {
//     // Явное приведение к числу на случай, если JSON вернул строки
//     const min = parseFloat(r.min);
//     const max = parseFloat(r.max);
//     return size > min && size <= max;
//   });

//   // 5. Если интервал не найден
//   if (!intervalRow) {
//     document.getElementById('result').innerHTML = `
//       <div class="error">
//         ❌ Не найден допуск для размера <strong>${size} мм</strong> и посадки <strong>${fit.toUpperCase()}</strong>.<br>
//         <small>Проверьте, поддерживается ли этот интервал в загруженной кассете знаний.</small>
//       </div>
//     `;
//     return;
//   }

//   // 6. Всё ок — показываем результат
//   const upper = intervalRow.upper / 1000; // в мм
//   const lower = intervalRow.lower / 1000;

//   document.getElementById('result').innerHTML = `
//     <div class="tolerance-result">
//       <strong>Допуск:</strong><br>
//       Верхнее: ${upper > 0 ? '+' : ''}${upper.toFixed(3)} мм<br>
//       Нижнее: ${lower > 0 ? '+' : ''}${lower.toFixed(3)} мм<br>
//       <span class="tolerance-range">Поле допуска: ${(upper - lower).toFixed(3)} мм</span>
//     </div>
//   `;

//   // Показать кнопку "Показать в таблице"
//   document.getElementById('showTableBtn').style.display = 'block';
// }
// Расчет допуска (обновленная версия)
// async function calculate() {
//   const sizeInput = document.getElementById('size').value;
//   const size = parseFloat(sizeInput);
//   const fit = document.getElementById('fit').value;
...
//     // Визуализация
//     setTimeout(() => updateVisualization(upper, lower, size), 100);
//     // updateVisualization(upper, lower, size);
    
//     // Показываем кнопку таблицы
//     document.getElementById('showTableBtn').style.display = 'block';
...
async function loadCompaniesForStats() {
  ...
  //         // ✅ НОВОЕ: Автозагрузка для напоминаний
...
//       return data.response; // ← Внимание: .response вместо .choices[0].message.content
//     } catch (error) {
...
//   // Функция для страницы допусков
// async function askAIaboutTolerance() {
//     const size = document.getElementById('size').value;
//     const fit = document.getElementById('fit').value;
    
//     if (!size || !fit) {
//       alert('Введите размер и выберите посадку');
//       return;
//     }
...
//     try {
//       // Загружаем контекст из кассеты
//       const fits = await loadKnowledgeData('fits.json');
//       const context = JSON.stringify({ fits }, null, 2);
  
//       const prompt = `Рассчитай допуск для Ø${size}${fit} по ГОСТ 25346. 
//       Покажи верхнее и нижнее отклонение в мм. Объясни, что это значит для инспектора.`;
  
//       const answer = await callAI(prompt, context);
//       responseDiv.innerHTML = `<strong>🤖 Ответ ИИ:</strong><br>${answer}`;
//     } catch (error) {
...
//     const systemPrompt = `Ты - инженерный ассистент для старшего инспектора ОТК в нефтегазовой отрасли.
    
//   Функции:
//   1. Расчет допусков по ГОСТ 25346
//   2. Поиск по нормативным документам  
//   3. Генерация шаблонов (NCR, акты, отчеты)
//   4. Проверка комплектности документации
//   5. Сравнение стандартов (API 6A, ГОСТ и др.)
//   6. Перевод технических формулировок
  
//   Данные доступны:
//   ${context}
  
//   Отвечай точно, с ссылками на стандарты. Не выдумывай.`;
...
// // Автоматически скрывать панель через 5 секунд после ввода ключа
// function saveApiKey() {
...
//       const answer = await callAI(prompt, context);
//       responseDiv.innerHTML = `<strong>🤖 Ответ ИИ:</strong><br>${answer}`;
//       responseDiv.style.display = 'block'; // На случай если display:none остался
//     } catch (error) {
//       responseDiv.innerHTML = `❌ Ошибка: ${error.message}`;
//       responseDiv.style.display = 'block';
//     }
...
//       alert('Загрузка кассеты...');
      
//       // TODO: Распаковка ZIP и сохранение файлов
//       // Пока просто сообщаем об успехе
//       alert(`Кассета "${file.name}" загружена!`);
      
//     } catch (error) {
//       console.error('Ошибка загрузки:', error);
//       alert('Ошибка загрузки кассеты');
//     }
//   });
...
//   async function translatePhrase() {
//     const translations = await (await fetch('data/translations.json')).json();
//     const phrase = document.getElementById('phraseSelect').value;
//     const eng = translations[phrase];
...
//   async function compareStandards() {
//     const comparisons = await (await fetch('data/comparison.json')).json();
//     const key = document.getElementById('compareSelect').value;
//     const data = comparisons[key];
...
//   async function searchStandard() {
//     const query = document.getElementById('query').value.toLowerCase();
//     const standards = await (await fetch('data/standards.json')).json();
//     let result = '';
...
//     const itemId = document.getElementById('itemId').value;
//     const type = document.getElementById('ncrType').value;
//     const templates = await (await fetch('data/ncr_templates.json')).json();
//     const template = templates[type];
...
//   async function loadCompany() {
//     const companies = await (await fetch('data/companies.json')).json();
//     const name = document.getElementById('companySelect').value;
//     const c = companies[name];
...
//   async function checkDocs() {
//     const checklist = await (await fetch('data/checklist.json')).json();
//     const type = document.getElementById('productType').value;
//     const required = checklist[type] || [];
...
//     // const materials = await (await fetch('data/materials.json')).json();
...
//         Тип: ${mat.type}<br>
//         Применение: ${mat.applications.join(', ')}<br>
//         Диапазон: ${mat.temp_range} °C<br>
//         Аналог: ${mat.analog}
//       `;
//     }
//   }
