...
//       e.preventDefault();  // ⭐ ЭТО ОБЯЗАТЕЛЬНО для <a href="#">
    
//       console.log("1. До загрузки", Potrace.info);
    
//       Potrace.loadImageFromUrl("https://raw.githubusercontent.com/kilobtye/kilobtye.github.io/12b9baa94028e19fd85f4d4214a183adadf12c26/potrace/yao.jpg");
...
//       console.log("3. После setParameter", Potrace.info);
...
//     //   console.log("2. После загрузки", Potrace.info);
...
//     //   Potrace.setParameter({
//     //     // 1. turdsize — размер "мусорных" пятен, которые будут удалены
//     //     //    Чем больше — тем более гладкая картинка, но могут исчезнуть мелкие детали
//     //     //    Мин: 0, Макс: ∞, Обычно: 0–5
//     //     turdsize: 0,
      
//     //     // 2. turnpolicy — как решать, куда поворачивать в неоднозначных местах
//     //     //    Варианты:
//     //     //      "black"    — выбирать путь по чёрным пикселям
//     //     //      "white"    — избегать чёрных
//     //     //      "left"     — всегда поворачивать налево
//     //     //      "right"    — всегда направо
//     //     //      "minority" — поворачивать в сторону редких пикселей (часто лучше всего)
//     //     //      "majority" — поворачивать в сторону большинства
//     //     //      "random"   — случайно
//     //     turnpolicy: "white",
      
//     //     // 3. optcurve — включать ли оптимизацию кривых Безье?
//     //     //    true → линии станут плавными (слишком "плавятся")
//     //     //    false → останутся угловатыми, ближе к пиксельной сетке (чётче!)
//     //     //    Если хочешь резкие линии — ставь false
//     //     optcurve: false,
      
//     //     // 4. alphamax — максимальное значение "кривизны", при котором отрезок считается прямым
//     //     //    Чем выше — тем больше сглаживания
//     //     //    Мин: 0, Макс: ~2+, Обычно: 0–1
//     //     //    Совет: ставь 0, если не хочешь сглаживания вообще
//     //     alphamax: 0,
      
//     //     // 5. opttolerance — насколько точно кривые должны повторять форму
//     //     //    Чем меньше — тем точнее, но больше точек
//     //     //    Чем больше — тем проще фигура, но теряются детали
//     //     //    Мин: 0.01, Макс: 0.5, Обычно: 0.2
//     //     opttolerance: 0.01
...
//   };
  function handleFiles(files) {
    // Шаг 1: Загружаем выбранный файл в Potrace
    Potrace.loadImageFromFile(files[0]);
  
    // Шаг 2: Настраиваем параметры векторизации
    Potrace.setParameter({
      // --- 1. turdsize ---
      // Минимальный размер пятна (в пикселях), которое будет игнорироваться
      // 0 = не удалять ничего; 2–5 = убрать мусор
      // Совет: ставь 0–1 для детализированных изображений
      turdsize: 1,
  
      // --- 2. turnpolicy ---
      // Как решать, куда поворачивать в спорных местах
      // "minority" — поворачивать в сторону менее частого цвета (часто лучше)
      // "white" — избегать чёрных пикселей (если фон чёрный — может помочь)
      // "black", "left", "right" — другие стратегии
      // Для мягкого контура: попробуй "minority"
      turnpolicy: "minority",
  
      // --- 3. optcurve ---
      // Включать ли оптимизацию кривых Безье?
      // true = можно делать плавные дуги
      // false = только прямые линии (ломаные)
      // Мы хотим немного сглаживания → true
      optcurve: true,
  
      // --- 4. alphamax ---
      // Чем выше — тем сильнее сглаживание углов
      // 0 = никакого сглаживания (острые углы)
      // 1 = максимальное сглаживание (очень мягкие формы)
      // Чтобы получить ~30% сглаживания → ставим 0.6
      alphamax: 0.9,
  
      // --- 5. opttolerance ---
      // Допуск при подборе кривых
      // Чем меньше — тем точнее, но больше точек
      // Чем больше — проще, но теряется форма
      // Для 30% сглаживания: ставим 0.1–0.15
      opttolerance: 0.1
    });
    ...
    Potrace.process(function(){
      ...
      // Генерируем SVG и показываем результат
      // Аргумент — масштаб (1 = норма, 3 = увеличено)
      displaySVG(1);
    });
  }
  // function handleFiles(files) {
